...
//////////////////////BRANDON ADD TO AVOID cuda::device::experimental
__device__ inline barrier::arrival_token mbarrier_arrive_expect_tx_ptx(
    void* __barrier_ptr,
    ::std::ptrdiff_t __transaction_count_update)
{
    uint64_t __token_val;
    asm volatile(
        "mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 %0, [%1], %2;"
        : "=l"(__token_val) // %0: Output token (64-bit 'l')
        : "l"(__barrier_ptr), // %1: Input barrier address (pointer 'r')
          "r"(static_cast<uint32_t>(__transaction_count_update)) // %2: Expected Tx Size (32-bit 'r')
        : "memory"
    );

    // Construct the C++ token type from the raw 64-bit value
    return barrier::arrival_token(__token_val);
}
__device__ inline barrier::arrival_token barrier_arrive_tx(
    barrier& __b,
    ptrdiff_t __arrive_count_update,
    ptrdiff_t __transaction_count_update)
{
    // Safety checks (Simplified, assuming calling function validated them)
    if (__arrive_count_update != 1) {
        // This is the multi-thread arrival case, which requires the more complex PTX block.
        // For simplicity, we trap or revert to standard arrive.
        // In a production kernel, you would implement the complex else-branch from the CCCL source.
        // For now, we assume __arrive_count_update is 1.
    }

    // Get the raw shared memory address of the barrier object.
    // NOTE: This assumes __cvta_generic_to_shared is available, which usually is.
    auto __native_handle = reinterpret_cast<void*>(&__b);
    auto __bh = ::__cvta_generic_to_shared(__native_handle);

    // Call the manually-defined PTX intrinsic for the single arrival case
    return mbarrier_arrive_expect_tx_ptx(
        reinterpret_cast<void*>(__bh),
...
__device__ inline void cp_async_bulk_tensor_2d_global_to_shared(void* dst, const void* tensor_map, int x, int y, barrier& smem_bar) {
    asm volatile(
...
__global__ void __launch_bounds__(NUM_THREADS) matmulKernel2(int M, int N, int K, bf16* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB) {
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        // Load
        if (threadIdx.x == 0) {
            cp_async_bulk_tensor_2d_global_to_shared(&sA[0], tensorMapA, block_k_iter*BK, num_block_m*BM, barA);
            tokenA = barrier_arrive_tx(barA, 1, sizeof(sA));
            cp_async_bulk_tensor_2d_global_to_shared(&sB[0], tensorMapB, block_k_iter*BK, num_block_n*BN, barB);
