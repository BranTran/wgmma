...
__device__ void wgmma256(float d[16][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma128(float d[8][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
    uint64_t desc_a = make_smem_desc(&sA[0]);
    uint64_t desc_b = make_smem_desc(&sB[0]);
    #pragma unroll 10
    for(unsigned long long i=0; i < iterations; i++){
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma64(float d[4][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
    uint64_t desc_a = make_smem_desc(&sA[0]);
    uint64_t desc_b = make_smem_desc(&sB[0]);
    #pragma unroll 10
    for(unsigned long long i=0; i < iterations; i++){
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma32(float d[2][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
    uint64_t desc_a = make_smem_desc(&sA[0]);
    uint64_t desc_b = make_smem_desc(&sB[0]);
    #pragma unroll 10
    for(unsigned long long i=0; i < iterations; i++){
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma16(float d[1][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
    uint64_t desc_a = make_smem_desc(&sA[0]);
    uint64_t desc_b = make_smem_desc(&sB[0]);
    #pragma unroll 10
    for(unsigned long long i=0; i < iterations; i++){
...
__global__ void __launch_bounds__(NUM_THREADS) matmulKernel128(int M, int N, int K, uint16_t* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, int *DB, unsigned long long iterations) {
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        ...
        for (int m_it = 0; m_it < B_WG_M/WGMMA_M; ++m_it) {
            ...
            for (int k_it = 0; k_it < BK/WGMMA_K; ++k_it) {
                wgmma128<1, 1, 1, 0, 0>(d[m_it], &wgmma_sA[k_it*WGMMA_K], &sB[k_it*WGMMA_K], iterations);
            }
        }
...
void runKernel128(int M, int N, int K, uint16_t *A, uint16_t *B, uint16_t *C, int *DB, unsigned long long iterations) {
    ...
    auto* kernel = DB ? matmulKernel128<BM, BN, BK, NUM_THREADS, true>
            : matmulKernel128<BM, BN, BK, NUM_THREADS, false>;
