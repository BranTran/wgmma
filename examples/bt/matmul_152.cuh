...
__device__ void wgmma152(float d[10][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
...
__global__ void __launch_bounds__(NUM_THREADS) matmulKernel152(int M, int N, int K, uint16_t* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, int *DB, unsigned long long iterations) {
    ...
    //BT: 152 not divisible by 16, hardcode value. WARNING ignoring padding breaks correctness
    float d[B_WG_M/WGMMA_M][10][8];
    memset(d, 0, sizeof(d));
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        ...
        for (int m_it = 0; m_it < B_WG_M/WGMMA_M; ++m_it) {
            ...
            for (int k_it = 0; k_it < BK/WGMMA_K; ++k_it) {
                wgmma152<1, 1, 1, 0, 0>(d[m_it], &wgmma_sA[k_it*WGMMA_K], &sB[k_it*WGMMA_K], iterations);
            }
        }
...
void runKernel152(int M, int N, int K, uint16_t *A, uint16_t *B, uint16_t *C, int *DB, unsigned long long iterations) {
    constexpr int BM = 128;
    constexpr int BN = 152;
    constexpr int BK = 64;
    ...
    auto* kernel = DB ? matmulKernel152<BM, BN, BK, NUM_THREADS, true>
            : matmulKernel152<BM, BN, BK, NUM_THREADS, false>;
