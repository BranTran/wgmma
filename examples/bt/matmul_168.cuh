...
__device__ void wgmma168(float d[11][8], uint16_t* sA, uint16_t* sB, unsigned long long iterations) {
...
__global__ void __launch_bounds__(NUM_THREADS) matmulKernel168(int M, int N, int K, uint16_t* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, int *DB, unsigned long long iterations) {
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        ...
        for (int m_it = 0; m_it < B_WG_M/WGMMA_M; ++m_it) {
            ...
            for (int k_it = 0; k_it < BK/WGMMA_K; ++k_it) {
                wgmma168<1, 1, 1, 0, 0>(d[m_it], &wgmma_sA[k_it*WGMMA_K], &sB[k_it*WGMMA_K], iterations);
            }
        }
...
void runKernel168(int M, int N, int K, uint16_t *A, uint16_t *B, uint16_t *C, int *DB, unsigned long long iterations) {
    constexpr int BM = 128;
    constexpr int BN = 168;
    constexpr int BK = 64;
    ...
    auto* kernel = DB ? matmulKernel168<BM, BN, BK, NUM_THREADS, true>
            : matmulKernel168<BM, BN, BK, NUM_THREADS, false>;
