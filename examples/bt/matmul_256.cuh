...
__global__ void __launch_bounds__(NUM_THREADS) matmulKernel256(int M, int N, int K, uint16_t* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, int *DB, unsigned long long iterations) {
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        ...
        for (int m_it = 0; m_it < B_WG_M/WGMMA_M; ++m_it) {
            ...
            for (int k_it = 0; k_it < BK/WGMMA_K; ++k_it) {
                wgmma256<1, 1, 1, 0, 0>(d[m_it], &wgmma_sA[k_it*WGMMA_K], &sB[k_it*WGMMA_K], iterations);
            }
        }
...
void runKernel256(int M, int N, int K, uint16_t *A, uint16_t *B, uint16_t *C, int *DB, unsigned long long iterations) {
    ...
    auto* kernel = DB ? matmulKernel256<BM, BN, BK, NUM_THREADS, true>
            : matmulKernel256<BM, BN, BK, NUM_THREADS, false>;
